...
export function AdminApp({ user, session, onSignOut, onRoleSwitch }: AdminAppProps) {
  const [activeTab, setActiveTab] = useState("dashboard");
  const [isMobileMenuOpen, setIsMobileMenuOpen] = useState(false);

  const navigation = [
    { id: "dashboard", label: "Dashboard", icon: BarChart3, color: "red" },
    { id: "users", label: "User Management", icon: Users, color: "orange" },
    { id: "subscriptions", label: "Subscription Management", icon: CreditCard, color: "purple" },
    { id: "security", label: "Security & Logs", icon: Shield, color: "blue" },
  ];

  const renderContent = () => {
    switch (activeTab) {
      case "users":
        return <UserManagementPage user={user} session={session} />;
      case "subscriptions":
        return <SubscriptionManagementPage user={user} session={session} />;
      case "security":
        return <AdminSecurityPage user={user} session={session} />;
      default:
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-red-50 via-white to-orange-50">
      {/* Header */}
      <header className="w-full px-4 md:px-6 py-4 bg-white/90 backdrop-blur-sm border-b border-red-100 sticky top-0 z-50">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center space-x-3">
            <div className="flex items-center space-x-2 px-3 py-2 bg-red-50 rounded-lg">
              ...
              <span className="text-sm text-red-900 font-medium">{user?.name}</span>
        ...
        {/* Mobile Navigation */}
        {isMobileMenuOpen && (
          <div className="lg:hidden mt-4 pb-4 border-t border-red-100">
