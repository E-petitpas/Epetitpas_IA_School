...
export const StudentApp: React.FC = () => {
  ...
  const renderContent = () => {
    switch (activeTab) {
      case "history":
        return <QuestionHistory user={user} session={{ user }} />;
      case "settings":
        return <StudentSettingsPage user={user} session={{ user }} onUpdate={fetchUserUsage} />;
      default:
        return <StudentDashboard user={user} session={{ user }} onUsageUpdate={fetchUserUsage} dailyUsage={dailyUsage} />;
    }
  };
